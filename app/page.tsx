    ...
    return (
        <main className={"flex min-h-screen flex-col items-center justify-center p-4"}>
            <div className={"w-full max-w-md space-y-8"}>
                ...
                {/*TODO: I'll need to make a component to stop repeating this errors*/}
                {wasDestroyed && <div className={"bg-red-950/50 border border-red-900 p-4 text-center rounded-md"}>
                    <p className={"text-red-500 text-sm font-bold"}>ROOM DESTROYED</p>
                    <p className={"text-zinc-500 text-xs mt-1"}>All messages were permanently deleted.</p>
                </div>
                }
                {error === "room-not-found" && <div className={"bg-red-950/50 border border-red-900 p-4 text-center rounded-md"}>
                    <p className={"text-red-500 text-sm font-bold"}>ROOM NOT FOUND</p>
                    <p className={"text-zinc-500 text-xs mt-1"}>This room may have expired or never existed.</p>
                </div>
                }
                {error === "room-full" && <div className={"bg-red-950/50 border border-red-900 p-4 text-center rounded-md"}>
